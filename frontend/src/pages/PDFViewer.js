...
const PDFViewer = ({ fileUrl }) => {
    ...
    const fetchNotes = async () => {
        try {
            const response = await fetch('http://localhost:5001/api/notes?fileUrl=' + encodeURIComponent(fileUrl));
            ...
            if (message !== '') {
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        setNotes(currentNotes => [...currentNotes, savedNote]);// Add note to local state
                        props.cancel(); // Close the highlight popup
                    } else {
    ...
    return (
        ...
        >
            ...
            >
                ...
                {notes.length === 0 && <div style={{ textAlign: 'center',color: '#000',  }}>There is no note</div>}
                {notes.map((note) => (
                    <div
                        key={note.id}
                        style={{
