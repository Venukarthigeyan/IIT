...
import { useHistory } from "react-router-dom"; // Updated for react-router-dom 5.3.4
...
const SearchBooks = () => {
  ...
  const [selectedBookTitle, setSelectedBookTitle] = useState(""); // Selected book title
 ...
 const books = {
  ...
  Biology: ["Cell Biology", "Plant and Animal Life", "Genetics Fundamentals"],
  ...
  const handleClassChange = (cls) => {
    ...
    setSelectedSubject(""); // Reset subject and book title
    setSelectedBookTitle("");
    setPdfUrl(""); // Reset PDF URL
    setIsSearchClicked(false); // Reset search state
  };
  ...
  const handleBookTitleChange = (title) => {
    ...
    const pdfPath = `/pdfs/${selectedClass}/${selectedSubject}/${title}.pdf`; // Construct PDF URL
    setPdfUrl(pdfPath); // Set the PDF URL
    console.log("PDF Path:", pdfPath);

    setIsSearchClicked(false); // Reset search state before clicking search
  };

  const handleSearchClick = () => {
    if (pdfUrl) {
      window.open(pdfUrl, "_blank"); // This opens the PDF in a new tab
    } else {
      setIsSearchClicked(true); // Fallback if PDF URL is not available yet
    }
  };
    ...
    >
      <Container
        ...
        style={{
          ...
          position: "relative", // Make container relative to position button inside
        }}
      >
        ...
        >
          <Button
            variant="contained"
            color="primary"
            onClick={() => history.push("/dashboard")} // Using history.push instead of navigate
            style={{
        ...
        {isSearchClicked && pdfUrl && (
          <Box style={{ marginTop: "30px", textAlign: "center" }}>
            <Typography variant="h5">Your Book:</Typography>
            <iframe
              src={pdfUrl}
              width="100%"
              height="600px"
              style={{ border: "1px solid #ddd" }}
              title="Book PDF" // Adding the title property
            />
          </Box>
        )}
      </Container>
